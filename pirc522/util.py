                                                                               ...
                                                                               c=c+chr(data[x])
                                                                                ...
                                                                                if x==14:
                                                                                    ...
                                                                                    if x==15:
                                                                                        ...
                                                                                        print (c)
                                                                                        return c
    def dex_ascci(self,data,block_address):
     if block_address==8:
        Nombre=self.codificacion(data)
        return Nombre
     else:
        if block_address==9:
            Sexo=self.codificacion(data)
            return True
        else:
            if block_address==10:
                Edad=self.codificacion(data)
                return True
            else:
                if block_address==12:
                    Identificacion=self.codificacion(data)
                    return True
                else:
                    if block_address==13:
                        Cargo=self.codificacion(data)
                        return True
                    else:
                        if block_address==14:
                            Celular=self.codificacion(data)
                            return True
                        else:
                            if block_address==16:
                                Correo1=self.codificacion(data)
                                return True
                            else:
                                if block_address==17:
                                    Correo2=self.codificacion(data)
                                    Correo= Correo1 + Correo2
                                    return True

                                else:
                                        Colegio=self.codificacion(data)
                                        con = mysql.connector.connect(user="pi",password="bsgp4444",host="127.0.0.1",database="Salon Tecno")
                                        cursor=con.cursor()
                                        cursor.execute("INSER INTO Salon Tecno (Nombre,Sexo,Edad,Identificacion,Cargo,Celular,Correo,Colegio) VALUES (Nombre,Sexo,Edad,Identificacion,Cargo,Celular,Correo,Colegio)")
                                        con.close()
                                        return True
